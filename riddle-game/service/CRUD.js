...
export function createFileDB(path, ObjData) {
    const d = readFileDB(path);
    if (Array.isArray(d) && d[0] === "" || d[0] === null) {
        const strData = convertsArrToString(ObjData);
...
//The function converts an object from an array to a string.
function convertsArrToString(ObjToEnter) {
    if (ObjToEnter !== null) {
        const strObj = JSON.stringify(ObjToEnter);
        return dataString;
    }
}

// fs.appendFile - זה יהרוס את הגייסון